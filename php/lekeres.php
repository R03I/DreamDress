	...
	class ruhak extends dbConnection
	{
		...
		function termek_megjelenit($array){
			...
			foreach ($array as $key => $row) {
				...
				"<a class=\"h6 text-dark \" href=\"termek.php?termek=$row[0]\" >". $row[1] . "</a>" . "<a href=\"termek.php?termek=$row[0]\">" . "<img class=\"img-thumbnail w-100\" src=\" $row[5] \" alt=\"$row[1]\"> " . "</a>" . "<h6>" . $row[4] . " ft" . "</h6>" . 
				"<button  type=\"button\" id=\"$item._button\" onclick=\"cart('$item')\" value=\"$row[4]\" class=\"btn btn-dark ruha w-50\">Kosárba</button>".
				"</div>
